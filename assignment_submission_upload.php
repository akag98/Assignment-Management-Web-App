		...
		if ($uploadOk == 0) {
			...
			if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
				...
				if ($_SERVER["REQUEST_METHOD"] == "POST") {
					...
					echo $file_name;
					if (!empty($_POST["assignment_name"]) && !empty($_POST["assignment_id"]) && $uploadOk==1) {
						$sql="INSERT INTO `submission` (`id`, `sid`, `assignment_name`, `question_id`, `class`, `Address`) VALUES (NULL, '".$_SESSION['rno']."', '".$assignment_name."' , '".$assignment_id."','".$_SESSION['class']."' , '".$file_name."')";
